...
async function startHisoka() {
  ...
  client.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      await client.sendMessage(owner + '@s.whatsapp.net', {
        text: '*ɢsᴅ ʙᴏᴛᴡᴀ sᴇᴄsᴇssғᴜʟʟʏ ᴄᴏɴɴᴇᴄᴛᴇᴅ ᴛᴏ sᴇʀᴠᴇʀ*',
      });
  ...
  client.cMod = (jid, copy, text = '', sender = client.user.id, options = {}) => {
    ...
    if (copy.key.remoteJid.includes('@s.whatsapp.net')) return sender;
    else return client.serializeM(msg);
  };

  client.relayWAMessage = async (mkey, jid, options) => {
    const mek = { key: { ...mkey }, message: options };
    return await client.sendMessage(jid, mek);
  };

  client.useHere = async (jid) => {
    let info = await client.infoMe(jid);
    if (info && info.viewOnce) {
      return client.sendMessage(jid, { viewOnce: true }, {
        contextInfo: { mentionedJid: [info.jid] },
      });
    }
    return false;
  };

  client.sendLocation = (jid, loc, quoted = '', options = {}) =>
    client.sendMessage(jid, { location: loc, ...options }, { quoted });

  client.sendContacts = (jid, contacts, quoted = '', options = {}) =>
    client.sendMessage(jid, { contacts, ...options }, { quoted });
  ...
  client.getBlockedIds = async () =>
    (await client.getBlockList()).map((contact) => contact.id);

  client.unblockUser = async (jid) =>
    await client.contactBlock(jid, 'remove');

  client.blockUser = async (jid) =>
    await client.contactBlock(jid, 'add');

  client.unsendMessage = (jid, messageId) =>
    client.modifyChat(jid, messageId, 'clear');
}
